...
function safeBackupPath(value) {
  const dir = process.env.ADMIN_BACKUP_DIR;
...
async function createBackup() {
  const dir = process.env.ADMIN_BACKUP_DIR;
  if (!dir) throw new Error('ADMIN_BACKUP_DIR تنظیم نشده است.');
  const dbName = currentDbName();
  if (!dbName) throw new Error('نام دیتابیس از DATABASE_URL قابل تشخیص نیست.');
  fs.mkdirSync(dir, { recursive: true });
  const filename = `roniya-ci-${new Date().toISOString().replace(/[:.]/g, '-')}.bak`;
  ...
  try {
    ...
    await prisma.$executeRawUnsafe(`BACKUP DATABASE [${escapedDb}] TO DISK=@p1 WITH INIT, CHECKSUM`, filePath);
    const stat = fs.statSync(filePath);
    if (!stat.size) throw new Error('فایل Backup خالی است.');
...
async function validateBackup(id) {
  const rows = await prisma.$queryRawUnsafe(`SELECT TOP 1 id,filePath,status,sizeBytes FROM dbo.AdminBackupJob WHERE id=@p1`, id);
  const row = rows[0];
  if (!row || row.status !== 'completed') return { valid: false, reason: 'Backup کامل‌شده پیدا نشد.' };
  const filePath = safeBackupPath(row.filePath);
  if (!filePath) return { valid: false, reason: 'مسیر Backup خارج از محدوده مجاز است.' };
  try {
    if (!fs.existsSync(filePath) || fs.statSync(filePath).size <= 0) return { valid: false, reason: 'فایل Backup وجود ندارد یا خالی است.' };
    await prisma.$executeRawUnsafe(`RESTORE VERIFYONLY FROM DISK=@p1 WITH CHECKSUM`, filePath);
    return { valid: true, filePath, sizeBytes: fs.statSync(filePath).size };
  } catch (error) {
    return { valid: false, reason: String(error.message || error).slice(0, 500) };
  }
}
...
router.post('/preflight', async (req, res) => {
  ...
  const workflowRunId = String(body.workflowRunId || '').trim();
  ...
  if (channel === 'stable' && process.env.ADMIN_REQUIRE_BACKUP_FOR_PRODUCTION_DEPLOY !== 'false') {
    ...
    try {
      ...
      const gate = { gateToken, backupId: backup.id, commitSha, channel, version, workflowRunId, expiresAt: Date.now() + 30 * 60 * 1000 };
      ...
      return res.json({ success: true, data: { gateToken, backupId: backup.id, backupPath: backup.filePath, backupSizeBytes: backup.sizeBytes, backupValid: true, commitSha, channel, version, workflowRunId, expiresAt: gate.expiresAt } });
    } catch (error) {
      return res.status(503).json({ success: false, message: `Backup پیش از استقرار انجام نشد: ${String(error.message || error).slice(0, 500)}` });
    }
  }
...
router.post('/complete', async (req, res) => {
  ...
  const channel = String(body.channel || 'stable').toLowerCase();
  const version = String(body.version || '').slice(0, 100);
  const workflowRunId = String(body.workflowRunId || '').slice(0, 100);
  const gateToken = String(body.gateToken || '');
  if (!/^[0-9a-f]{40}$/i.test(commitSha)) return res.status(400).json({ success: false, message: 'Commit SHA نامعتبر است.' });
  await ensureTable();
  let gate = null;
  try {
    const rows = await prisma.$queryRawUnsafe(`SELECT TOP 1 [value] FROM dbo.GlobalSetting WHERE [key]=N'deployment.ci.gate'`);
    gate = rows[0]?.value ? JSON.parse(rows[0].value) : null;
  } catch (_) {}
  if (status === 'completed' && process.env.ADMIN_REQUIRE_BACKUP_FOR_PRODUCTION_DEPLOY !== 'false') {
    ...
    const validation = await validateBackup(Number(gate.backupId));
    if (!validation.valid) return res.status(409).json({ success: false, message: 'Backup مرتبط با استقرار دیگر معتبر نیست.', validation });
  }
  ...
  if (status === 'completed') {
    try { await prisma.$executeRawUnsafe(`DELETE FROM dbo.GlobalSetting WHERE [key]=N'deployment.ci.gate'`); } catch (_) {}
  }
