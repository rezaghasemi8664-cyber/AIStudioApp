...
function isUsableToken(v) {
  return !!(v && typeof v === 'string' && v.trim() && v !== 'null' && v !== 'undefined');
}

/**
 * Extract ACCESS token only (Header > Cookie > Query)
 * NOTE: Do NOT use refreshToken for protected API auth.
 */
function extractToken(req) {
  const authHeader = req.headers.authorization || req.headers.Authorization;

  // پشتیبانی از حالت استاندارد Bearer
  ...
  // fallback: اگر کلاینت اشتباهاً خود توکن را بدون Bearer فرستاد
  if (isUsableToken(authHeader) && !/^Bearer\s+/i.test(authHeader)) {
    return { token: String(authHeader).trim(), source: 'header_raw' };
  }

  const cookieToken =
    (req.cookies && (req.cookies.accessToken || req.cookies.access_token || req.cookies.token)) ||
    (req.signedCookies &&
      (req.signedCookies.accessToken || req.signedCookies.access_token || req.signedCookies.token));

  if (isUsableToken(cookieToken)) return { token: cookieToken, source: 'cookie' };

  if (req.query && isUsableToken(req.query.token)) {
...
function buildUserFromDecoded(decoded) {
  ...
  // پشتیبانی از انواع payload رایج
  ...
  return {
    ...
    userId,
    sub: userId,
    username: decoded.username || decoded.user || decoded.name || null,
  ...
  try {
    ...
    if (!token) {
      return res.status(401).json({
        ...
        messageEn: 'Authentication token not provided.',
        code: 'NO_TOKEN',
      });
    }
    ...
    // فقط access token
    if (decoded?.type && String(decoded.type).toLowerCase() === 'refresh') {
      return res.status(401).json({
        success: false,
        message: 'نوع توکن نامعتبر است.',
        messageEn: 'Invalid token type for this endpoint.',
    ...
    if (!user) {
      return res.status(401).json({
        success: false,
        message: 'توکن فاقد اطلاعات کاربری معتبر است.',
    ...
    if ((process.env.NODE_ENV || 'development') !== 'production') {
      console.log(`[AUTH_MW] OK userId=${user.userId} role=${user.role} via=${source}`);
    }
    ...
    if (err?.name === 'NotBeforeError') {
      return res.status(401).json({
        success: false,
        message: 'توکن هنوز فعال نشده است.',
        messageEn: 'Token not active yet.',
...
module.exports.authenticateToken = authenticate; // ✅ alias مهم برای سازگاری
