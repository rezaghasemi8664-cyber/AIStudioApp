...
function buildResponseMarketData(analysisPayload,responseMarketMetrics,responsePriceHistory) {
  ...
  const liveClosingPrice=getClosingPrice(analysisPayload||{}); const liveLastPrice=getLastPrice(analysisPayload||{}); const closingPrice=liveClosingPrice>0?liveClosingPrice:(toNumber(responseMarketMetrics&&responseMarketMetrics.closingPrice,0)>0?toNumber(responseMarketMetrics&&responseMarketMetrics.closingPrice,0):toNumber(dailySummary.close,0)); const lastPrice=liveLastPrice>0?liveLastPrice:(toNumber(responseMarketMetrics&&responseMarketMetrics.lastPrice,0)>0?toNumber(responseMarketMetrics&&responseMarketMetrics.lastPrice,0):toNumber(dailySummary.last,0)); const tradedVolume=Math.max(toNumber(dailySummary.volume,0),toNumber(responseMarketMetrics&&responseMarketMetrics.tradedVolume,0),getTradedVolume(analysisPayload||{}));
