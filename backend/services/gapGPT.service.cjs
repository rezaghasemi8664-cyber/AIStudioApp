...
const FETCH_TIMEOUT_MS = Number(env.FETCH_TIMEOUT_MS || 30000);
...
function extractAnalysisDataQuality(input) {
  ...
  if (asPlainObject(source.analysisDataQuality)) {
    return source.analysisDataQuality;
  }
...
function safeJsonParse(raw) {
  if (typeof raw !== 'string') {
    throw new Error('AI_RESPONSE_NOT_STRING');
  }
...
function checkRateLimit(userId) {
  ...
  if (validRequests.length >= RATE_LIMIT_MAX) {
    ...
    throw new Error(
      'RATE_LIMIT_EXCEEDED. ' + Math.ceil(Math.max(remainingMs, 0) / 1000) + 's remaining'
    );
  }
}
...
async function executeWithRetry(fn, maxRetries) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    try {
      ...
      logger.warn(
        '[GapGPT] Retry ' + (attempt + 1) + '/' + (retries + 1) + ' after ' + delay + 'ms',
        {
          error: error.message,
...
function buildMessages(input) {
  ...
  const qualityContext = buildPromptContext(input);
  if (qualityContext) {
    messages.push({
      role: 'system',
      content: JSON.stringify(qualityContext)
    });
  }
...
function extractMessageContent(response) {
  const choices =
    response && response.data && Array.isArray(response.data.choices)
...
function buildFinalMeta(traceId, input, parsedSuccessfully) {
  return {
    traceId: traceId,
    model: env.GAPGPT_MODEL || 'gpt-4o-mini',
    timestamp: new Date().toISOString(),
...
async function runAnalysis(input, userId) {
  ...
  const traceId =
    'gapgpt-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
  ...
  try {
    ...
    if (!ontology || !ontology.version) {
      throw new Error('ONTOLOGY_VERSION_MISSING');
    }
    ...
    const finalResult = Object.assign({}, validated, {
      meta: Object.assign(
        {},
        validated && validated.meta && typeof validated.meta === 'object' ? validated.meta : {},
        buildFinalMeta(traceId, input, parsedResult.parsedSuccessfully)
      )
    });
    ...
    return buildFallbackResult(traceId, error.message, {
      meta: buildFinalMeta(traceId, input, false)
    });
  }
}
...
function healthCheck() {
  const apiKeyConfigured =
    !!env.GAPGPT_API_KEY && env.GAPGPT_API_KEY !== 'gapgpt_xxx';
  const ontologyVersion = ontology && ontology.version ? ontology.version : null;
...
async function analyzeBulk(symbols, userId) {
  if (!Array.isArray(symbols) || symbols.length === 0) {
    throw new Error('SYMBOLS_MUST_BE_ARRAY');
  }

  if (symbols.length > 50) {
    throw new Error('MAX_50_SYMBOLS_PER_BULK');
  }
  ...
  for (let i = 0; i < symbols.length; i += 1) {
    ...
    try {
      results[symbol] = await runAnalysis(
        { type: 'bulk', symbol: symbol, context: 'quick-scan' },
        userId
      );
    } catch (error) {
      results[symbol] = buildFallbackResult(
        'gapgpt-bulk-' + Date.now() + '-' + i,
        error.message
      );
    }
  }
