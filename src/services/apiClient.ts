...
function deriveMarketSentiment(data: any): 'positive' | 'negative' | 'neutral' {
  ...
  const priceChange = toFinite(marketData.priceChangePercent ?? marketData.lastPriceChangePercent ?? metrics.priceChangePercent ?? data?.lastChangePercent);
  ...
  if (hasFlow) return totalFlow > 0 ? 'positive' : totalFlow < 0 ? 'negative' : 'neutral';
  return 'neutral';
}
...
async function enrichStockAnalysis(payload: any, symbol: string, headers: Record<string, string>): Promise<any> {
  ...
  try {
    const supplementalResponse = await fetch(buildUrl(`/analyze/stock-data/${encodeURIComponent(symbol)}?historyCount=30`), {
...
export async function request<T>(method: string, endpoint: string, data?: unknown, customHeaders?: Record<string, string>): Promise<ApiResponse<T>> {
  const url = buildUrl(endpoint);
  const headers: Record<string, string> = { ...customHeaders };
  const token = getStoredToken();
  if (token) headers.Authorization = `Bearer ${token}`;
  if (data !== undefined && method !== 'GET' && method !== 'HEAD' && !(data instanceof FormData)) headers['Content-Type'] = 'application/json';
  const options: RequestInit = { method, headers, credentials: 'include' };
  if (data !== undefined && method !== 'GET' && method !== 'HEAD') options.body = data instanceof FormData ? data : JSON.stringify(data);
  ...
  try {
    ...
    let payload: any = contentType.includes('application/json') ? await response.json() : { message: await response.text() };

    if (endpoint.replace(/^\/+/, '').toLowerCase() === 'analyze/stock' && payload?.success === true) {
      const symbol = typeof data === 'object' && data !== null && 'symbol' in data ? String((data as Record<string, unknown>).symbol ?? '').trim() : '';
    ...
    return { success: false, message: errorMessage, error: errorMessage, statusCode: 0 };
  }
}
...
export function get<T>(endpoint: string, headers?: Record<string, string>): Promise<ApiResponse<T>> { return request<T>('GET', endpoint, undefined, headers); }
export function post<T>(endpoint: string, data?: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>> { return request<T>('POST', endpoint, data, headers); }
export function put<T>(endpoint: string, data?: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>> { return request<T>('PUT', endpoint, data, headers); }
export function patch<T>(endpoint: string, data?: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>> { return request<T>('PATCH', endpoint, data, headers); }
export function del<T = unknown>(endpoint: string, headers?: Record<string, string>): Promise<ApiResponse<T>> { return request<T>('DELETE', endpoint, undefined, headers); }
export async function upload<T>(endpoint: string, formData: FormData, customHeaders?: Record<string, string>): Promise<ApiResponse<T>> { return request<T>('POST', endpoint, formData, customHeaders); }
