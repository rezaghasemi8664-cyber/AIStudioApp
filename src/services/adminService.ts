...
export interface AdminUpdateUserPayload {
  username?: string; password?: string; email?: string | null; name?: string | null;
  firstName?: string | null; lastName?: string | null; phone?: string | null; mobile?: string | null;
  nationalId?: string | null; bio?: string | null; avatar?: string | null;
  isActive?: boolean; analysisLimit?: number | null; analysisLimit24h?: number | null;
  subscriptionStart?: string | null; subscriptionEnd?: string | null; subscriptionDays?: number | null;
  subscriptionMonths?: number | null; isSubscriptionActive?: boolean;
}
...
export async function getRoles():Promise<AdminRole[]>{try{const r=await apiClient.get<AdminRole[]>('/admin-rbac/roles');if(!r?.success||!Array.isArray(r.data))return [];return r.data.filter(role=>Number.isInteger(role?.id)&&role.id>0).map(role=>({id:role.id,name:String(role.name||'').trim(),title:role.title??null,userCount:role.userCount,permissionCount:role.permissionCount}));}catch(e){console.warn('[adminService] getRoles failed:',e);return [];}}
...
export async function updateUser(id:UserId,p:AdminUpdateUserPayload):Promise<AdminUserListItem|null>{try{return unwrapOrNull(await apiClient.put<AdminUserListItem>(`${USERS_BASE}/${encodeUserId(id)}`,omitUndefined({username:normalizeString(p.username),password:normalizeString(p.password),email:normalizeNullableString(p.email),name:normalizeNullableString(p.name),firstName:normalizeNullableString(p.firstName),lastName:normalizeNullableString(p.lastName),phone:normalizeNullableString(p.phone),mobile:normalizeNullableString(p.mobile),nationalId:normalizeNullableString(p.nationalId),bio:normalizeNullableString(p.bio),avatar:normalizeNullableString(p.avatar),isActive:normalizeBoolean(p.isActive),analysisLimit:normalizeNumber(p.analysisLimit),analysisLimit24h:normalizeNumber(p.analysisLimit24h),subscriptionStart:normalizeNullableString(p.subscriptionStart),subscriptionEnd:normalizeNullableString(p.subscriptionEnd),subscriptionDays:normalizeNumber(p.subscriptionDays),subscriptionMonths:normalizeNumber(p.subscriptionMonths),isSubscriptionActive:normalizeBoolean(p.isSubscriptionActive)})));}catch(e){throw new Error(errorMessage(e,'ویرایش کاربر با خطا مواجه شد.'));}}
export async function toggleUserActive(id:UserId,isActive:boolean):Promise<AdminUserListItem|null>{try{const r=await apiClient.patch<AdminUserListItem>(`${USERS_BASE}/${encodeUserId(id)}/toggle-active`,{isActive:!!isActive});return unwrapOrNull(r);}catch(e){throw new Error(errorMessage(e,'تغییر وضعیت فعال بودن کاربر با خطا مواجه شد.'));}}
...
export default {getDashboard,getUsers,getUser,getRoles,createUser,updateUser,toggleUserActive,changeUserRole,deleteUser,updateSubscription,resetUserPassword};
