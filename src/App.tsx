  ...
  ({ tab, label, icon, alertType, disabled = false, locked = false, activeTab, onTabClick }) => {
    ...
    return (
      <button
        ...
        className={`flex shrink-0 items-center gap-2 px-4 py-3 text-sm font-semibold whitespace-nowrap rounded-t-lg transition-all duration-300 focus:outline-none ${alertClasses[alertType]} ${hoverBg} ${border} ${disabled || locked ? 'opacity-50 cursor-not-allowed' : ''}`}
        style={{
  ...
  return (
    <>
      ...
      <div className={`app-shell min-h-screen text-[var(--app-color)] flex flex-col p-3 sm:p-4 lg:p-6 transition-colors duration-300 ${!isOnline && !currentUser.isAdmin ? 'pt-12' : ''}`}>
        ...
        {activeTab === 'dashboard' && <header data-style-id="header" data-style-name="هدر اصلی" className="app-header flex items-center justify-between px-3 sm:px-5 py-3 border-b -mx-3 sm:-mx-4 lg:-mx-6 mb-4" style={{ backgroundColor: 'var(--header-bg)', borderColor: 'var(--header-border-color)' }}>
        ...
        <nav className="app-nav mb-4 sm:mb-6" aria-label="ناوبری اصلی"><div className="app-nav-scroll flex flex-nowrap items-center overflow-x-auto overflow-y-hidden overscroll-x-contain whitespace-nowrap border-b border-gray-200 dark:border-gray-700 [scrollbar-width:thin] [-webkit-overflow-scrolling:touch]">
