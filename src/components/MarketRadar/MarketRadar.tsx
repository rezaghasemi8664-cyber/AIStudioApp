...
import MarketRadarHistory from './MarketRadarHistory';
...
const signedNumber = (value: number | null | undefined): string => { if (value === null || value === undefined || !Number.isFinite(value)) return '—'; return `${value > 0 ? '+' : value < 0 ? '−' : ''}${fa(Math.abs(value))}`; };
...
const moverRows = (rows: MarketRadarMover[], empty: string): React.ReactNode => rows.length ? rows.map((row) => <div key={row.symbol} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 border-b border-white/5 py-3 last:border-b-0"><span className="truncate font-bold text-slate-900 dark:text-white">{row.symbol}</span><span className="text-xs text-slate-500">{fa(row.volume)}</span><span className={`text-sm font-black ${tone(row.changePercent)}`}>{signedPercent(row.changePercent)}</span></div>) : <div className="py-6 text-center text-sm text-slate-500">{empty}</div>;
const SectorRows: React.FC<{ rows: MarketRadarSector[] }> = ({ rows }) => <div className="space-y-2">{rows.length ? rows.map((row) => <div key={row.name} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 rounded-xl bg-white/[0.025] px-3 py-2"><span className="truncate text-sm font-semibold text-slate-900 dark:text-white">{row.name}</span><span className="text-xs text-slate-500">{fa(row.symbols)} نماد</span><span className={`text-sm font-bold ${tone(row.changePercent)}`}>{signedPercent(row.changePercent)}</span></div>) : <div className="py-5 text-center text-sm text-slate-500">داده صنعت در دسترس نیست</div>}</div>;
...
const MarketRadar: React.FC = () => {
  ...
  const netBreadth = breadth ? breadth.positive - breadth.negative : null;
  const directionalTotal = breadth ? breadth.positive + breadth.negative : 0;
  const directionalPressure = breadth && directionalTotal > 0 ? ((breadth.positive - breadth.negative) / directionalTotal) * 100 : null;
  const positiveWidth = directionalPressure === null ? 50 : 50 + directionalPressure / 2;
  const negativeWidth = 100 - positiveWidth;

  return <section dir="rtl" className="page-shell market-radar-page space-y-5">
    <header className="flex flex-col gap-3 rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 sm:flex-row sm:items-center sm:justify-between">
      <div><div className="flex flex-wrap items-center gap-3"><h1 className="text-xl font-black text-[var(--color-text-primary)]">رادار بازار</h1><span className={`rounded-full px-3 py-1 text-xs font-bold ${data.marketOpen ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-500/10 text-slate-400'}`}>{data.marketOpen ? 'بازار باز' : 'بازار بسته'}</span><span className={`rounded-full px-3 py-1 text-xs font-bold ${stale ? 'bg-amber-500/10 text-amber-300' : data.dataStatus === 'LIVE' ? 'bg-emerald-500/10 text-emerald-300' : 'bg-slate-500/10 text-slate-400'}`}>{stale ? 'داده ذخیره‌شده' : data.dataStatus === 'LIVE' ? 'داده زنده' : 'داده در دسترس نیست'}</span><span className="text-xs text-slate-500">منبع: {data.source || '—'}</span></div><p className="mt-1 text-xs text-slate-500">آخرین دریافت: {lastUpdated} · داده‌های واقعی و محاسبات قطعی؛ بدون وابستگی به هوش مصنوعی</p></div>
    ...
    <article className="rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5"><div className="mb-5"><h2 className="font-black text-[var(--color-text-primary)]">نقشه حرارتی صنایع</h2><p className="mt-1 text-xs text-slate-500">مقایسه بصری تغییرات صنایع؛ رنگ شدت حرکت را نشان می‌دهد و اندازه کارت بر اساس تعداد نمادهاست.</p></div><IndustryHeatmap rows={breadth?.sectors.rows ?? []} /></article>
