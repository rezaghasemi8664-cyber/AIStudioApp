...
const moverRows = (rows: MarketRadarMover[], empty: string): React.ReactNode => rows.length ? rows.map((row) => <div key={row.symbol} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 border-b border-white/5 py-3 last:border-b-0"><span className="truncate font-bold text-white dark:text-white">{row.symbol}</span><span className="text-xs text-slate-500">{fa(row.volume)}</span><span className={`text-sm font-black ${tone(row.changePercent)}`}>{signedPercent(row.changePercent)}</span></div>) : <div className="py-6 text-center text-sm text-slate-500">{empty}</div>;
