...
function ChartLine({ values, width = 760, height = 270, padding = 32, onHover }: { values: Array<number | null>; width?: number; height?: number; padding?: number; onHover?: (index: number, x: number, y: number) => void }) {
  ...
  const toPoint = ({ value, index }: { value: number; index: number }) => {
    const x = padding + (index / Math.max(values.length - 1, 1)) * (width - padding * 2);
    const y = height - padding - ((value - min) / span) * (height - padding * 2);
    return { x, y };
  };
  const points = valid.map(toPoint).map(({ x, y }) => `${x},${y}`).join(' ');
  return <>
    ...
    {valid.map((item) => { const point = toPoint(item); return <circle key={item.index} cx={point.x} cy={point.y} r="5" fill="currentColor" stroke="var(--color-surface)" strokeWidth="2" className="cursor-pointer" onMouseEnter={() => onHover?.(item.index, point.x, point.y)} onFocus={() => onHover?.(item.index, point.x, point.y)} tabIndex={0} aria-label={`نقطه ${item.index + 1}`} />; })}
  </>;
}
...
const MarketRadarHistory: React.FC = () => {
  ...
  const firstValid = values.find((value) => value !== null && Number.isFinite(value)) as number | undefined;
  ...
  const rangeChange = firstValid !== undefined && lastValue !== null && lastValue !== undefined && Number.isFinite(lastValue) && firstValid !== 0 ? ((lastValue - firstValid) / Math.abs(firstValid)) * 100 : null;
      ...
      <div className="mt-5 overflow-x-auto rounded-2xl border border-white/5 bg-black/5 p-2">
        <div className="relative min-w-[680px]">
          <svg viewBox="0 0 760 270" className="h-72 w-full text-sky-400" role="img" aria-label={`نمودار تاریخی ${selectedMetric.label}`} onMouseLeave={() => setHovered(null)}>
            <ChartLine values={values} onHover={(index, x, y) => setHovered({ index, x, y })} />
            {hovered && hoveredPoint && <g pointerEvents="none">
              ...
              <rect x={Math.min(Math.max(hovered.x - 100, 8), 552)} y={Math.max(hovered.y - 72, 8)} width="200" height="58" rx="12" fill="var(--color-surface)" stroke="currentColor" strokeOpacity="0.25" />
              <text x={Math.min(Math.max(hovered.x, 108), 652)} y={Math.max(hovered.y - 45, 35)} textAnchor="middle" className="fill-[var(--color-text-primary)] text-[11px] font-bold">{dateFa(hoveredPoint.timestamp)}</text>
              <text x={Math.min(Math.max(hovered.x, 108), 652)} y={Math.max(hovered.y - 25, 55)} textAnchor="middle" className="fill-sky-300 text-[11px] font-black">{valueLabel(hoveredValue)}</text>
            </g>}
          </svg>
        </div>
      </div>
