...
const currencyLabel=(c:string)=>c==='IRT'?'تومان':c==='IRR'?'ریال':c;
...
const AdminPaymentsPanel:React.FC<Props>=({onComplete=()=>undefined})=>{
 ...
 const load=useCallback(async(targetPage=page,filterOverride?:typeof filters)=>{
   ...
   try{
     ...
     const q:paymentsService.PaymentsQuery={page:targetPage,limit:20,search:activeFilters.search,status:activeFilters.status,gateway:activeFilters.gateway,currency:activeFilters.currency,from:activeFilters.from,to:activeFilters.to};
     if(activeFilters.userId)q.userId=Number(activeFilters.userId);
 ...
 const setStatus=()=>{if(!selected)return;const next=statusForm.status;const current=selected.status;if(next===current){setError('وضعیت جدید با وضعیت فعلی یکسان است.');return;}if((next==='paid'||next==='refunded')&&!statusForm.referenceNo.trim()){setError('برای پرداخت‌شده یا مستردشده، شماره مرجع الزامی است.');return;}if(current==='refunded'){setError('تراکنش مستردشده دیگر قابل تغییر نیست.');return;}if(current==='paid'&&next!=='refunded'){setError('تراکنش پرداخت‌شده فقط به «مستردشده» قابل تغییر است.');return;}if(current==='pending'&&!['paid','failed','cancelled'].includes(next)){setError('تراکنش در انتظار فقط می‌تواند پرداخت‌شده، ناموفق یا لغوشده شود.');return;}if((current==='failed'||current==='cancelled')&&next!=='pending'){setError('تراکنش ناموفق یا لغوشده ابتدا باید به «در انتظار» برگردد.');return;}if(!window.confirm(`وضعیت تراکنش #${selected.id} از «${labels[current]||current}» به «${labels[next]||next}» تغییر کند؟`))return;void runAction('set-status',{transactionId:selected.id,status:next,referenceNo:statusForm.referenceNo});setSelected(null);};
 const reset=()=>{const empty={userId:'',search:'',status:'',gateway:'',currency:'',from:'',to:''};setFilters(empty);void load(1,empty);};
 ...
 return <div className="space-y-5" dir="rtl">
   ...
   <div className={card}>
    <div className="flex items-center justify-between gap-3"><h3 className="font-bold">فیلتر تراکنش‌ها</h3><button onClick={reset} className="rounded-lg border px-3 py-2 text-xs">پاک‌کردن فیلترها</button></div>
   ...
   <div className={card}>
    ...
    <div className="mt-3 overflow-auto"><table className="w-full min-w-[1180px] text-sm"><thead><tr className="border-b"><th className="p-3 text-right">شناسه</th><th className="p-3 text-right">کاربر</th><th className="p-3 text-right">مبلغ</th><th className="p-3 text-right">درگاه</th><th className="p-3 text-right">Authority</th><th className="p-3 text-right">وضعیت</th><th className="p-3 text-right">مرجع</th><th className="p-3 text-right">زمان</th><th className="p-3 text-right">عملیات</th></tr></thead><tbody>{result?.items.map(r=><tr key={r.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800/40"><td className="p-3" dir="ltr">{r.id}</td><td className="p-3" dir="ltr">{r.userId}</td><td className="p-3 font-semibold">{fmt(r.amount)} {currencyLabel(r.currency)}</td><td className="p-3">{r.gateway||'—'}</td><td className="p-3 max-w-[180px] truncate" dir="ltr" title={r.authority||''}>{r.authority||'—'}</td><td className="p-3"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs dark:bg-slate-800">{labels[r.status]||r.status}</span></td><td className="p-3" dir="ltr">{r.referenceNo||'—'}</td><td className="p-3">{dateFmt(r.createdAt)}</td><td className="p-3"><button onClick={()=>{setSelected(r);setStatusForm({status:r.status==='paid'?'refunded':r.status==='pending'?'paid':'pending',referenceNo:r.referenceNo||''});}} className="rounded-lg border px-3 py-1.5 text-xs">جزئیات / وضعیت</button></td></tr>)}</tbody></table>{!result?.items.length&&<div className="py-8 text-center text-gray-500">تراکنشی مطابق فیلترها پیدا نشد.</div>}</div>
