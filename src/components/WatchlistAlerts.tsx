...
const WatchlistAlerts: React.FC<Props> = ({ isOnline }) => {
  ...
  return <div dir="rtl" className="space-y-5">
    ...
    <div className="rounded-2xl border border-[var(--color-border)] overflow-hidden">
      ...
      {history.length === 0 ? <div className="p-8 text-center text-gray-500">هنوز هیچ هشداری فعال نشده است.</div> : <div className="divide-y divide-[var(--color-border)]">{history.map(item => { const unread = !readIds.includes(item.id); const expanded = expandedId === item.id; return <div key={item.id} className={unread ? 'bg-amber-50/70 dark:bg-amber-950/10' : ''}><button type="button" onClick={() => { markRead(item.id); setExpandedId(expanded ? null : item.id); }} className="w-full text-right px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition"><div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-4"><span className={`w-2.5 h-2.5 rounded-full shrink-0 ${unread ? 'bg-amber-500' : 'bg-gray-300 dark:bg-gray-600'}`} /><span className="text-xs text-gray-500 whitespace-nowrap">{new Date(item.triggeredAt).toLocaleString('fa-IR')}</span><span className="font-black">{item.symbol}</span><span className="text-sm">{item.logic || 'AND'} · {item.conditions?.length || 1} شرط</span><span className="mr-auto text-xs text-cyan-600">{expanded ? 'بستن جزئیات' : 'مشاهده جزئیات'}</span></div></button>{expanded && <div className="px-4 pb-4 space-y-3"><div className="rounded-xl bg-gray-50 dark:bg-gray-800/70 p-3 grid grid-cols-2 md:grid-cols-5 gap-3 text-xs"><div><span className="text-gray-500">قیمت آخر</span><b className="block mt-1">{item.snapshot?.lastPrice != null ? item.snapshot.lastPrice.toLocaleString('fa-IR') : '—'}</b></div><div><span className="text-gray-500">درصد آخر</span><b className="block mt-1">{item.snapshot?.lastChangePercent != null ? `${item.snapshot.lastChangePercent.toLocaleString('fa-IR')}٪` : '—'}</b></div><div><span className="text-gray-500">حجم</span><b className="block mt-1">{item.snapshot?.volume != null ? item.snapshot.volume.toLocaleString('fa-IR') : '—'}</b></div><div><span className="text-gray-500">قیمت پایانی</span><b className="block mt-1">{item.snapshot?.closePrice != null ? item.snapshot.closePrice.toLocaleString('fa-IR') : '—'}</b></div><div><span className="text-gray-500">زمان Snapshot</span><b className="block mt-1">{item.snapshot?.capturedAt ? new Date(item.snapshot.capturedAt).toLocaleString('fa-IR') : '—'}</b></div></div>{item.conditions?.length ? <div className="rounded-xl border border-[var(--color-border)] p-3 space-y-2"><div className="font-bold text-sm">نتیجه شروط</div>{item.conditions.map((condition, i) => <div key={`${item.id}-condition-${i}`} className="flex flex-wrap items-center gap-2 text-xs"><span>{metricLabel(condition.metric)}</span><span>{operatorLabel(condition.operator)}</span><b>{Number(condition.threshold).toLocaleString('fa-IR')}</b><span>→ {condition.value == null ? 'داده موجود نیست' : Number(condition.value).toLocaleString('fa-IR')}</span><span className={`px-2 py-1 rounded-lg ${condition.matched ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300' : 'bg-gray-100 dark:bg-gray-800'}`}>{condition.matched ? 'برقرار' : 'برقرار نیست'}</span></div>)}</div> : null}</div>}</div>; })}</div>}
    </div>
  </div>;
};
