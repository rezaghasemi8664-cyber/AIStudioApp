...
const AdminPluginsPanel:React.FC=()=>{
 const inputRef=useRef<HTMLInputElement>(null); const [file,setFile]=useState<File|null>(null); const [items,setItems]=useState<pluginService.AdminPlugin[]>([]); const [loading,setLoading]=useState(true); const [busy,setBusy]=useState(false); const [error,setError]=useState<string|null>(null); const [message,setMessage]=useState<string|null>(null);
 const load=useCallback(async()=>{setLoading(true);setError(null);try{setItems(await pluginService.listPlugins());}catch(e){setError(e instanceof Error?e.message:'دریافت افزونه‌ها ناموفق بود.');}finally{setLoading(false);}},[]);
 ...
 return <div className="space-y-5">
  ...
  <div className="rounded-2xl border border-[var(--card-border-color)] bg-[var(--card-bg)] p-5 shadow-sm"><div className="flex items-center justify-between"><h3 className="font-bold">افزونه‌های نصب‌شده</h3><span className="rounded-full bg-gray-100 px-3 py-1 text-xs dark:bg-gray-800">{items.length.toLocaleString('fa-IR')} افزونه</span></div>
   {loading?<div className="py-10 text-center text-gray-500">در حال دریافت فهرست افزونه‌ها…</div>:items.length===0?<div className="py-10 text-center text-gray-500">هنوز افزونه‌ای نصب نشده است.</div>:<div className="mt-4 overflow-x-auto"><table className="w-full text-right text-sm"><thead><tr className="border-b border-[var(--card-border-color)]"><th className="p-3">نام</th><th className="p-3">شناسه</th><th className="p-3">نسخه</th><th className="p-3">وضعیت</th><th className="p-3">تاریخ نصب</th></tr></thead><tbody>{items.map(p=><tr key={p.id} className="border-b border-[var(--card-border-color)] last:border-0"><td className="p-3 font-semibold">{p.name}<div className="text-xs text-gray-500">{p.description}</div></td><td className="p-3 font-mono text-xs ltr" dir="ltr">{p.id}</td><td className="p-3">{p.version}</td><td className="p-3"><span className={`rounded-full px-2.5 py-1 text-xs ${p.status==='installed'?'bg-green-100 text-green-700':'bg-red-100 text-red-700'}`}>{p.status==='installed'?'نصب‌شده':'نامعتبر'}</span></td><td className="p-3 whitespace-nowrap">{new Date(p.installedAt).toLocaleString('fa-IR')}</td></tr>)}</tbody></table></div>}
  </div>
 </div>;
};
