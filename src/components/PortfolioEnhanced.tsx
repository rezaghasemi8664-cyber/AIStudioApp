...
import PortfolioRiskContributionAnalysis from './PortfolioRiskContributionAnalysis';
...
const PortfolioEnhanced: React.FC<PortfolioEnhancedProps> = ({ onAlertChange, currentUser, isOnline }) => {
  const [activeSection, setActiveSection] = useState<'portfolio' | 'portfolioIntelligence' | 'portfolioHistory' | 'portfolioReturn' | 'portfolioRisk' | 'portfolioCorrelation' | 'portfolioRiskContribution' | 'watchlist' | 'alerts' | 'profile' | 'smartScore' | 'codal' | 'codalSensitive' | 'codalAnalysis'>('portfolio');
  ...
  return (
    <div dir="rtl" className="max-w-7xl mx-auto space-y-5">
      <div className="flex items-center gap-1 overflow-x-auto border-b border-[var(--color-border)]">
        ...
        <button type="button" onClick={() => setActiveSection('portfolioRisk')} className={tabClass('portfolioRisk')}>ریسک سبد</button>
        <button type="button" onClick={() => setActiveSection('portfolioCorrelation')} className={tabClass('portfolioCorrelation')}>همبستگی سبد</button>
        <button type="button" onClick={() => setActiveSection('portfolioRiskContribution')} className={tabClass('portfolioRiskContribution')}>سهم ریسک نمادها</button>
