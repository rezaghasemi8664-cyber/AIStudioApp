...
const PortfolioEnhanced: React.FC<PortfolioEnhancedProps> = ({ onAlertChange, currentUser, isOnline }) => {
  const [activeSection, setActiveSection] = useState<'portfolio' | 'portfolioIntelligence' | 'portfolioHistory' | 'portfolioReturn' | 'portfolioRisk' | 'watchlist' | 'alerts' | 'profile' | 'smartScore' | 'codal' | 'codalSensitive' | 'codalAnalysis'>('portfolio');
