...
const PortfolioEnhanced: React.FC<PortfolioEnhancedProps> = ({ onAlertChange, currentUser, isOnline }) => {
  const [activeSection, setActiveSection] = useState<'portfolio' | 'watchlist' | 'alerts'>('portfolio');
  ...
  return (
    <div dir="rtl" className="max-w-6xl mx-auto space-y-5">
      ...
      {activeSection === 'portfolio' ? <Portfolio onAlertChange={onAlertChange} currentUser={currentUser} isOnline={isOnline} /> : activeSection === 'watchlist' ? <PortfolioWatchlists currentUser={currentUser} isOnline={isOnline} /> : <WatchlistAlerts isOnline={isOnline} />}
    </div>
  );
};
