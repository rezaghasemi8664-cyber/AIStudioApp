...
const AdminAuditPanel: React.FC<Props> = ({ modules }) => {
  ...
  const moduleOptions = useMemo(() => modules.length ? modules : Object.keys(moduleTitles).map(moduleKey => ({ id: moduleKey as adminControlService.AdminModuleKey, title: moduleTitles[moduleKey] })), [modules]);
  ...
  return <div dir="rtl" className="space-y-4">
    <div className="rounded-2xl border border-[var(--card-border-color)] bg-[var(--card-bg)] p-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        <select value={String(draft.moduleKey || '')} onChange={e => setDraft(v => ({ ...v, moduleKey: e.target.value || undefined }))} className="rounded-xl border bg-transparent px-3 py-2 text-sm"><option value="">همه ماژول‌ها</option>{moduleOptions.map(m => <option key={m.id} value={m.id}>{m.title || moduleTitles[m.id]}</option>)}</select>
