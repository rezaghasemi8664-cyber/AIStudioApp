...
interface Props { modules?: Array<{ id: adminControlService.AdminModuleKey; title: string }>; }
const moduleTitles: Record<string,string>={dashboard:'داشبورد مدیریتی',users:'مدیریت کاربران',subscriptions:'اشتراک‌ها',analysis:'تحلیل‌ها',market:'بازار',scalping:'نوسان‌گیری',ai:'هوش مصنوعی',prompts:'پرامپت‌ها',history:'تاریخچه',notifications:'اطلاع‌رسانی',monitoring:'مانیتورینگ',reports:'گزارش‌ها',security:'امنیت',settings:'تنظیمات',maintenance:'تعمیرات',updates:'بروزرسانی',backup:'پشتیبان‌گیری',payments:'پرداخت‌ها',roles:'نقش‌ها',audit:'گزارش Audit Log',sessions:'نشست‌ها',api:'APIها',infrastructure:'زیرساخت'};
...
const AdminAuditPanel:React.FC<Props>=({modules=[]})=>{
 ...
 const [filters,setFilters]=useState<adminControlService.AdminAuditFilters>({}),[draft,setDraft]=useState<adminControlService.AdminAuditFilters>({}),[stats,setStats]=useState<adminControlService.AdminAuditStats|null>(null);
 const limit=25;
 const load=useCallback(async(nextOffset=offset,activeFilters=filters)=>{setLoading(true);setError(null);try{const page=await adminControlService.getAuditPage(limit,nextOffset,activeFilters);setRows(page.data);setTotal(page.pagination.total);setOffset(page.pagination.offset);setStats(page.stats||null);}catch(e){setError(e instanceof Error?e.message:'دریافت Audit Log ناموفق بود.');}finally{setLoading(false);}},[filters,offset]);
 useEffect(()=>{void load(0,filters);},[]);
 const moduleOptions=useMemo(()=>modules.length?modules:Object.keys(moduleTitles).map(id=>({id:id as adminControlService.AdminModuleKey,title:moduleTitles[id]})),[modules]);
 const pageNumber=Math.floor(offset/limit)+1,pageCount=Math.max(1,Math.ceil(total/limit));
 const pageSuccessful=rows.filter(r=>r.statusCode!=null&&r.statusCode>=200&&r.statusCode<400).length;
 const pageFailed=rows.filter(r=>r.statusCode!=null&&r.statusCode>=400).length;
