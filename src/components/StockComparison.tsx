...
import { compareDeterministicStocks, compareStocksWithMarketData, type ComparisonMarketSnapshot, type DeterministicComparisonResult } from '../services/stockComparisonDataService';
...
const StockComparison: React.FC<StockComparisonProps> = ({ currentUser, isOnline }) => {
    ...
    const handleCompare = async (e: React.FormEvent) => {
        e.preventDefault();
        setLoading(true); setError(null); setResult(null); setSnapshots({}); setDeterministicResult(null);
        try {
            ...
            if (response.result.rows.length >= 2) setResult(normalizeComparisonResult(response.result, response.result.rows[0].symbol, response.result.rows[1].symbol));
        } catch (err: any) {
