...
const AnalysisCard: React.FC<{ title: string, content: string }> = ({ title, content }) => (
    <div>
        ...
        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap">{content || 'اطلاعاتی در این بخش از سرویس مقایسه دریافت نشد.'}</p>
    </div>
);
...
const normalizeComparisonResult = (raw: any, symbol1: string, symbol2: string): StockComparisonResult => {
    ...
    // New/expected structured response: keep it intact but guarantee both analysis objects exist.
    ...
    // Current GapGPT comparator response is legacy-shaped:
    // { winner, reason, scores, details }. Convert it to the UI contract.
            ...
            {result && (
                <div className="space-y-8 animate-fade-in">
                    <div className="p-6 rounded-lg shadow-md bg-indigo-50 dark:bg-indigo-900/40 border-l-4 border-indigo-500">
                        ...
                        <p className="text-gray-800 dark:text-gray-200 leading-relaxed">{result.final_recommendation || 'توصیه نهایی دریافت نشد.'}</p>
                    </div>
                    <div className="p-6 rounded-lg shadow-md bg-gray-50 dark:bg-gray-800/50">
                        ...
                        <p className="text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap">{result.comparison_summary || 'خلاصه مقایسه دریافت نشد.'}</p>
                    </div>

                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="p-6 rounded-lg shadow-md space-y-4 bg-white dark:bg-gray-800">
                            <h3 className="text-2xl font-bold text-cyan-700 dark:text-cyan-400">{result.symbol1_analysis?.recommendation || 'نگهداری'}: {symbol1}</h3>
                        ...
                        <div className="p-6 rounded-lg shadow-md space-y-4 bg-white dark:bg-gray-800">
                            <h3 className="text-2xl font-bold text-cyan-700 dark:text-cyan-400">{result.symbol2_analysis?.recommendation || 'نگهداری'}: {symbol2}</h3>
