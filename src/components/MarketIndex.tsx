...
const MarketIndex: React.FC<MarketIndexProps> = ({ isOnline }) => {
    ...
    return <div dir="rtl" data-style-id={showAsClosed ? 'market-index-closed' : 'market-index-open'} data-style-name={showAsClosed ? 'شاخص بازار بسته' : 'شاخص بازار باز'} data-style-props="bg,border,positive,negative" className="market-index-widget market-index-workstation w-full min-w-0 max-w-[460px] overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-lg shadow-black/10">
        <div className="flex flex-col items-stretch gap-2 border-b border-[var(--color-border)] px-3 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:px-4"><div className="min-w-0"><div className="text-[14px] font-bold text-[var(--color-text-primary)]">شاخص‌های بازار</div><div className="mt-0.5 text-[11px] font-medium text-slate-400">{dataMeta?.status === 'CACHED' ? 'آخرین داده معتبر ذخیره‌شده' : dataMeta?.status === 'LIVE' ? 'داده واقعی بازار' : 'وضعیت داده مشخص نیست'}</div></div><span className={`inline-flex w-fit shrink-0 items-center gap-2 rounded-full border px-3 py-1 text-[11px] font-bold leading-5 sm:text-[12px] ${showAsClosed ? 'border-red-500/20 bg-red-500/10 text-red-400' : 'border-emerald-500/20 bg-emerald-500/10 text-emerald-300'}`}><span className={`h-2 w-2 rounded-full ${showAsClosed ? 'bg-red-400' : 'bg-emerald-400'}`} />{showAsClosed ? 'بازار بسته' : 'بازار باز'}</span></div>
        <div className="market-index-values relative flex w-full flex-row items-stretch gap-0 px-1.5 py-4 sm:px-2"><div aria-hidden="true" className="pointer-events-none absolute right-1/2 top-4 bottom-4 w-px translate-x-1/2 bg-[var(--color-border)]" /><div className="market-index-column min-w-0 w-1/2 flex-[0_0_50%] px-0 sm:pl-5"><IndexDisplay name="شاخص کل" value={data.value} changeValue={data.changeValue} changePercent={data.changePercent} showIcon={!showAsClosed} /></div><div className="market-index-column min-w-0 w-1/2 flex-[0_0_50%] px-0 sm:pr-5"><IndexDisplay name="شاخص هم‌وزن" value={data.equalWeightedValue} changeValue={data.equalWeightedChangeValue} changePercent={data.equalWeightedChangePercent} showIcon={!showAsClosed} /></div></div>
    </div>;
};
