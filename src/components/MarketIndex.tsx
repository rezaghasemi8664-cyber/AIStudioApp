...
const MarketIndex: React.FC<MarketIndexProps> = ({ isOnline }) => {
    ...
    return <div dir="rtl" data-style-id={showAsClosed ? 'market-index-closed' : 'market-index-open'} data-style-name={showAsClosed ? 'شاخص بازار بسته' : 'شاخص بازار باز'} data-style-props="bg,border,positive,negative" className="market-index-widget market-index-workstation w-full min-w-0 max-w-[460px] overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-lg shadow-black/10">
        ...
        <div className="relative flex w-full flex-row items-stretch gap-0 px-1.5 py-4 sm:px-2"><div aria-hidden="true" className="pointer-events-none absolute right-1/2 top-4 bottom-4 w-px translate-x-1/2 bg-[var(--color-border)]" /><div className="min-w-0 w-1/2 flex-[0_0_50%] px-0 sm:pl-5"><IndexDisplay name="شاخص کل" value={data.value} changeValue={data.changeValue} changePercent={data.changePercent} showIcon={!showAsClosed} /></div><div className="min-w-0 w-1/2 flex-[0_0_50%] px-0 sm:pr-5"><IndexDisplay name="شاخص هم‌وزن" value={data.equalWeightedValue} changeValue={data.equalWeightedChangeValue} changePercent={data.equalWeightedChangePercent} showIcon={!showAsClosed} /></div></div>
    </div>;
};
