...
const formatNumber = (value: number | null | undefined): string => value === null || value === undefined || !Number.isFinite(value) ? '—' : new Intl.NumberFormat('fa-IR', { maximumFractionDigits: 2 }).format(value);
...
const MoverList: React.FC<{ title: string; items: DashboardMover[]; showVolume?: boolean }> = ({ title, items, showVolume = false }) => (
  <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm">
    <div className="mb-4 flex items-center justify-between"><h3 className="font-bold text-[var(--color-text-primary)]">{title}</h3><span className="text-xs text-slate-500">۱۰ نماد</span></div>
    {items.length === 0 ? <div className="rounded-xl bg-black/5 px-3 py-6 text-center text-sm text-slate-500 dark:bg-white/5">داده‌ای در دسترس نیست</div> : <div className="space-y-2">{items.map((item, index) => <div key={`${item.symbol}-${index}`} className="flex items-center justify-between rounded-xl px-3 py-2 hover:bg-black/5 dark:hover:bg-white/5"><div className="min-w-0"><div className="font-bold text-sm text-[var(--color-text-primary)]">{item.symbol}</div>{item.name && <div className="truncate text-xs text-slate-500">{item.name}</div>}</div><div className="text-left"><div className="text-sm font-bold tabular-nums text-[var(--color-text-primary)]">{formatNumber(item.price)}</div><div className="text-xs font-bold tabular-nums text-[var(--color-text-primary)]" dir="ltr">{showVolume ? `حجم ${formatNumber(item.volume)}` : formatPercent(item.changePercent)}</div></div></div>)}</div>}
  </section>
);
...
const Dashboard: React.FC<DashboardProps> = ({ isOnline = true, onNavigate, unreadAlertCount = 0, subscriptionDaysRemaining = null, subscriptionExpired = false }) => {
  ...
  return <div dir="rtl" className="space-y-5">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-black text-[var(--color-text-primary)]">داشبورد بازار</h1><p className="mt-1 text-sm text-slate-500">نمای کلی بازار، دارایی‌های شخصی و فعالیت صنایع</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-black/5 px-3 py-1 text-xs font-bold text-slate-500 dark:bg-white/5">{marketStatus}</span><button type="button" onClick={() => void load()} className="rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm font-bold text-[var(--color-text-primary)]">به‌روزرسانی</button></div></header>
    ...
    <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm"><div className="mb-4"><h3 className="font-bold text-[var(--color-text-primary)]">وضعیت صنایع و گروه‌ها</h3><p className="mt-1 text-xs text-slate-500">مرتب‌سازی بر اساس تغییرات روزانه؛ داده مستقیماً از سرویس بازار دریافت می‌شود.</p></div>{(data?.industries ?? []).length === 0 ? <div className="rounded-xl bg-black/5 px-3 py-6 text-center text-sm text-slate-500 dark:bg-white/5">اطلاعات صنایع در سرویس بازار در دسترس نیست.</div> : <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{data?.industries.map((industry) => <div key={industry.name} className="flex items-center justify-between rounded-xl border border-[var(--color-border)] px-3 py-3"><div><div className="font-bold text-sm text-[var(--color-text-primary)]">{industry.name}</div>{industry.symbolCount != null && <div className="text-xs text-slate-500">{formatNumber(industry.symbolCount)} نماد</div>}</div><div className={`font-black tabular-nums ${industry.changePercent >= 0 ? 'text-[var(--color-positive)]' : 'text-[var(--color-negative)]'}`}>{formatPercent(industry.changePercent)}</div></div>)}</div>}</section>

    <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm"><div className="mb-4"><h3 className="font-bold text-[var(--color-text-primary)]">دسترسی سریع</h3><p className="mt-1 text-xs text-slate-500">ابزارهای اصلی بدون وابستگی به هوش مصنوعی</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><button type="button" onClick={() => onNavigate?.('analysis')} className="rounded-xl border border-cyan-500/20 bg-cyan-500/10 p-4 text-right"><div className="font-black text-[var(--color-text-primary)]">تحلیل نماد</div><div className="mt-1 text-xs text-slate-500">بررسی یک سهم</div></button><button type="button" onClick={() => onNavigate?.('dailyFilters')} className="rounded-xl border border-[var(--color-border)] p-4 text-right"><div className="font-black text-[var(--color-text-primary)]">فیلتر و Screener</div><div className="mt-1 text-xs text-slate-500">پیدا کردن نمادهای مناسب</div></button><button type="button" onClick={() => onNavigate?.('portfolio')} className="rounded-xl border border-[var(--color-border)] p-4 text-right"><div className="font-black text-[var(--color-text-primary)]">سبد و دیده‌بان</div><div className="mt-1 text-xs text-slate-500">مدیریت دارایی و نمادها</div></button><button type="button" onClick={() => onNavigate?.('comparison')} className="rounded-xl border border-[var(--color-border)] p-4 text-right"><div className="font-black text-[var(--color-text-primary)]">مقایسه سهام</div><div className="mt-1 text-xs text-slate-500">مقایسه چند نماد</div></button></div></section>
  </div>;
};
