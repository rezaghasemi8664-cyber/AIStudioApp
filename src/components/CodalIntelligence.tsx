...
const CodalIntelligence: React.FC<Props> = ({ isOnline }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setResult(await getCodalReports({ symbol: symbol.trim(), from, to, limit }));
    } catch (e: any) {
      setError(e?.response?.data?.message || e?.message || 'دریافت اطلاعیه‌های کدال ناموفق بود.');
    ...
    <section dir="rtl" className="space-y-4">
      ...
      {result && <>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ['کل اطلاعیه‌ها', result.summary.reports],
            ['حسابرسی‌شده', result.summary.audited],
            ['دارای پیوست', result.summary.attachments],
            ['نوع اطلاعیه', Object.keys(result.summary.byType).length],
          ].map(([label, value]) => <div key={String(label)} className="rounded-2xl border border-[var(--color-border)] bg-white/80 dark:bg-gray-900/60 p-4"><div className="text-xs text-gray-500">{label}</div><div className="text-2xl font-black mt-1">{faNumber(Number(value))}</div></div>)}
        </div>

        <div className="rounded-2xl border border-[var(--color-border)] overflow-hidden bg-white/80 dark:bg-gray-900/60">
          <div className="px-4 py-3 border-b border-[var(--color-border)] font-black">اطلاعیه‌های کدال</div>
          {result.items.length === 0 ? <div className="p-6 text-center text-gray-500">اطلاعیه‌ای برای فیلتر انتخاب‌شده پیدا نشد.</div> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-right border-b border-[var(--color-border)]"><th className="p-3">عنوان</th><th className="p-3">نماد</th><th className="p-3">نوع</th><th className="p-3">تاریخ</th><th className="p-3">وضعیت</th></tr></thead><tbody>{result.items.map((item, index) => <tr key={`${item.symbol}-${item.publishDate}-${index}`} onClick={() => setSelected(item)} className="border-b border-[var(--color-border)] hover:bg-gray-50 dark:hover:bg-gray-800/60 cursor-pointer"><td className="p-3 font-bold min-w-[280px]">{item.title || '—'}</td><td className="p-3">{item.symbol || '—'}</td><td className="p-3">{item.reportType || '—'}</td><td className="p-3">{item.publishDate || '—'}</td><td className="p-3">{item.audited ? 'حسابرسی‌شده' : 'عادی'}{item.attachment ? ' • پیوست' : ''}</td></tr>)}</tbody></table></div>}
        </div>
      </>}

      {selected && <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setSelected(null)}><div onClick={e => e.stopPropagation()} className="w-full max-w-2xl max-h-[85vh] overflow-auto rounded-2xl bg-white dark:bg-gray-900 border border-[var(--color-border)] p-5"><div className="flex justify-between gap-4"><h3 className="font-black text-lg">جزئیات اطلاعیه</h3><button type="button" onClick={() => setSelected(null)} className="text-gray-500">بستن</button></div><div className="space-y-3 mt-5"><div><span className="text-gray-500">عنوان:</span> {selected.title || '—'}</div><div><span className="text-gray-500">شرکت:</span> {selected.companyName || '—'}</div><div><span className="text-gray-500">نماد:</span> {selected.symbol || '—'}</div><div><span className="text-gray-500">نوع:</span> {selected.reportType || '—'}</div><div><span className="text-gray-500">دوره:</span> {selected.period || '—'}</div><div><span className="text-gray-500">پایان دوره:</span> {selected.periodEnd || '—'}</div><div><span className="text-gray-500">تاریخ انتشار:</span> {selected.publishDate || '—'}</div>{selected.url && <a href={selected.url} target="_blank" rel="noreferrer" className="inline-block text-cyan-600 font-bold">مشاهده اطلاعیه در منبع</a>}</div></div></div>}
    </section>
  );
};
