...
async function history(symbol: string): Promise<Array<{ date: string; close: number }>> {
  ...
  return raw.map(row => ({ date: String(row.date ?? row.tradeDate ?? row.jalaliDate ?? row.timestamp ?? ''), close: num(row.close ?? row.closePrice ?? row.closingPrice ?? row.lastPrice) ?? 0 })).filter(row => row.date && row.close > 0);
}
...
const PortfolioIntelligence: React.FC<Props> = ({ isOnline }) => {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const portfolio = await portfolioService.getPortfolio(); if (!portfolio.length) { setHistoryPoints([]); return; }
      const rows = await Promise.all(portfolio.map(async item => { try { return { item, rows: await history(item.symbol) }; } catch (_) { return { item, rows: [] }; } }));
  ...
  const attribution = useMemo(() => {
    ...
    return [...valued].sort((a, b) => b.pnl - a.pnl).map(item => ({ ...item, contributionPercent: totalAbs > 0 ? (Math.abs(item.pnl) / totalAbs) * 100 : 0, shareOfTotalPnl: stats.pnl !== 0 ? (item.pnl / stats.pnl) * 100 : null }));
  }, [items, stats.pnl]);
