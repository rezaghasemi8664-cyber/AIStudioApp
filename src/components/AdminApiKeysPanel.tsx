...
const AdminApiKeysPanel: React.FC<Props> = ({ onComplete = () => undefined }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(Array.isArray(result) ? result as ApiKeyRow[] : []);
    } catch (e) {
  ...
  const stats = useMemo(() => ({
    total: rows.length,
    active: rows.filter(r => !r.isRevoked).length,
    revoked: rows.filter(r => Boolean(r.isRevoked)).length,
  }), [rows]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      const matchesStatus = statusFilter === 'all'
        || (statusFilter === 'active' && !r.isRevoked)
        || (statusFilter === 'revoked' && Boolean(r.isRevoked));
      const matchesQuery = !q || [r.id, r.userId, r.username, r.email, r.name, r.service]
  ...
  const revoke = async (id: number | string) => {
    ...
    setError(null);
    try {
      await adminActionsService.executeAction('api', 'revoke-api-key', { apiKeyId: Number(id) });
      await load();
      await onComplete();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'لغو کلید API ناموفق بود.');
    } finally {
      setBusyId(null);
    }
  };
  ...
  return <div dir="rtl" className="space-y-5">
    <div className="rounded-2xl border border-[var(--card-border-color)] bg-[var(--card-bg)] p-5">
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="rounded-xl border border-green-200 bg-green-50/50 p-4 dark:border-green-900/40 dark:bg-green-900/10"><div className="text-xs text-gray-500">کلیدهای فعال</div><div className="mt-1 text-2xl font-bold text-green-600">{stats.active.toLocaleString('fa-IR')}</div></div>
        <div className="rounded-xl border border-red-200 bg-red-50/50 p-4 dark:border-red-900/40 dark:bg-red-900/10"><div className="text-xs text-gray-500">کلیدهای لغوشده</div><div className="mt-1 text-2xl font-bold text-red-600">{stats.revoked.toLocaleString('fa-IR')}</div></div>
      </div>
      <div className="mt-4 grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-3">
        <input value={query} onChange={e => setQuery(e.target.value)} placeholder="جستجو بر اساس شناسه، کاربر، ایمیل یا نام کلید" className="rounded-xl border bg-transparent px-4 py-2.5 text-sm" />
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as StatusFilter)} className="rounded-xl border bg-[var(--card-bg)] px-4 py-2.5 text-sm">
          <option value="all">همه وضعیت‌ها</option><option value="active">فعال</option><option value="revoked">لغوشده</option>
        </select>
        <div className="rounded-xl bg-gray-50 dark:bg-gray-800/50 px-4 py-2.5 text-sm">نتایج: <b>{filtered.length.toLocaleString('fa-IR')}</b></div>
      </div>
    </div>
    {error && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}
    <div className="rounded-2xl border border-[var(--card-border-color)] bg-[var(--card-bg)] overflow-hidden">
      <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead><tr className="border-b bg-gray-50/70 dark:bg-gray-800/40"><th className="p-3 text-right">شناسه</th><th className="p-3 text-right">مالک</th><th className="p-3 text-right">نام کلید</th><th className="p-3 text-right">سرویس</th><th className="p-3 text-right">تاریخ ایجاد</th><th className="p-3 text-right">وضعیت</th><th className="p-3 text-right">عملیات</th></tr></thead>
