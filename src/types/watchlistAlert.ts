...
/**
 * AI-independent alert definition used by Watchlist and the Phase 5 Alert Engine.
 * The Watchlist only stores the contract; evaluation/execution belongs to Phase 5.
 */
...
export const WATCHLIST_ALERT_METRICS: ReadonlyArray<{
  value: WatchlistAlertMetric;
  label: string;
}> = [
...
export const WATCHLIST_ALERT_OPERATORS: ReadonlyArray<{
  value: WatchlistAlertOperator;
  label: string;
}> = [
...
/** Pure rule evaluator. No network, persistence, or AI dependency. */
export function evaluateWatchlistAlert(
